 ...
 * @author David Schaack (original)
 * Fixes: drag drift (circleCalculations), quiz layout (side-by-side with ECG)
 */
...
// Scroll wheel on the circle rotates the axis
...
function switchToCabrera() {
  if (currentSection === 'cabrera') return;
  // Move quiz panel back to main (outside .split)
  if (quiz.parentElement === split) main.appendChild(quiz);
...
function switchToQuiz() {
  ...
  // Move quiz panel INTO .split so it flexes beside #ecg
  split.appendChild(quiz);
...
function resizeDivs() {
  document.querySelectorAll("div.containerInner").forEach(e => resizeProportionally(e));
}
...
const styleCircle = (degree) => {
  ...
  else                                    { styleCircleParts(lt,   circleParts); displayLagetyp.style.backgroundColor = 'var(--lt)'; }
};
...
const stylecircleInt = (degree) => {
  ...
  else                                    { styleCircleParts(enNormal, circlePartsEn); displayLagetyp.style.backgroundColor = 'var(--it)'; }
};
